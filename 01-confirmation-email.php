<?php 
    ...
    $email_array = array(
        array("email"=>$sEmail,"name"=>$sName,"subject"=>$appSubject,"Body"=>$appBody,"altBody"=>$altappBody),
        array("email"=>$refEmail1,"name"=>$refName1,"subject"=>$refSubject,"Body"=>$refBody,"altBody"=>$altrefBody),
        array("email"=>$refEmail2,"name"=>$refName2,"subject"=>$refSubject,"Body"=>$refBody,"altBody"=>$altrefBody),
        array("email"=>$refEmail3,"name"=>$refName3,"subject"=>$refSubject,"Body"=>$refBody,"altBody"=>$altrefBody)
    );
	$replacements=array();
    // Loop through each email to be sent
    foreach ($email_array as $this_email){
        $replacements[$this_email["email"]]=array("{name}"=>$this_email["name"]);
        ...
        $message->setTo(array($this_email["email"]=>$this_email["name"]));
        $mailer->send($message);					
    }
